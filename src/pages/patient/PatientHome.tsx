      ...
      <section className="mt-6 sm:mt-8">
        <div className="flex items-start sm:items-center justify-between mb-3 sm:mb-4 gap-2">
          <div className="min-w-0">
            ...
            <p className="text-[10px] font-bold text-slate-400 uppercase mt-0.5 hidden sm:block">Top-rated specialties</p>
          </div>
          <button 
            onClick={() => navigate(ROUTES.PATIENT_SEARCH)} 
            className="text-xs font-black text-blue-600 hover:underline hover:text-blue-700 transition-colors flex items-center gap-1 whitespace-nowrap flex-shrink-0"
          >
          ...
          {loading ? (
            ...
            doctors.map((doctor) => (
              <motion.div 
                ...
                className="p-3 sm:p-4 bg-white rounded-2xl sm:rounded-3xl border border-slate-100 shadow-sm flex items-center gap-3 sm:gap-4 cursor-pointer relative overflow-hidden group hover:border-blue-200 transition-all"
              >
                <div className="relative flex-shrink-0">
                  <img src={doctor.photoUrl || "https://images.unsplash.com/photo-1559839734-2b71f153678e?auto=format&fit=crop&q=80&w=200&h=200"} className="w-12 sm:w-14 h-12 sm:h-14 rounded-lg sm:rounded-2xl object-cover border border-slate-100 shadow-sm" alt={doctor.name} />
                  <div className="absolute -bottom-1 -right-1 w-4 sm:w-4.5 h-4 sm:h-4.5 bg-emerald-500 border-2 border-white rounded-full flex items-center justify-center flex-shrink-0">
                     <div className="w-1.5 h-1.5 bg-white rounded-full animate-pulse" />
                  </div>
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1">
                    <Award className="w-3 sm:w-3.5 h-3 sm:h-3.5 text-blue-600 flex-shrink-0" />
                    <span className="text-[8px] sm:text-[9px] font-black text-blue-600 bg-blue-50 px-2 py-0.5 rounded-md uppercase tracking-wide whitespace-nowrap">
                      Certified
                    </span>
                  </div>
                  <h3 className="font-black text-slate-900 text-xs sm:text-sm leading-tight mt-1 truncate">{doctor.name}</h3>
                  <p className="text-slate-400 text-[9px] sm:text-[10px] font-bold uppercase tracking-wider truncate">{doctor.specialty}</p>
                </div>
                <ChevronRight className="w-4 h-4 text-slate-300 group-hover:text-blue-600 transition-colors ml-auto flex-shrink-0" />
              </motion.div>
            ))
          ) : (
            <div className="col-span-full text-center py-6 text-slate-400 text-xs">No clinic doctors on registry</div>
          )}
        </div>
      </section>

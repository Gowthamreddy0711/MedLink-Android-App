...
export const db = {
  ...
  // Coverage Requests
  getCoverageRequests: async (doctorId?: string): Promise<any[]> => {
    try {
      let q;
      if (doctorId) {
        q = query(collection(firestoreDb, 'coveragerequests'), where('toDoctorId', '==', doctorId));
      } else {
        q = collection(firestoreDb, 'coveragerequests');
      }
      const snapshot = await getDocs(q);
      const fsCov = snapshot.docs.map(doc => doc.data() as any);
      const localCov = localDb.getItem<any[]>('medlink_local_coveragerequests', []);
      const filteredLocal = doctorId ? localCov.filter(r => r.toDoctorId === doctorId) : localCov;
      ...
      console.warn('[FIRESTORE] getCoverageRequests failed, loading from LocalStorage:', err);
      const localCov = localDb.getItem<any[]>('medlink_local_coveragerequests', []);
      return doctorId ? localCov.filter(r => r.toDoctorId === doctorId) : localCov;
    }
  },

  createCoverageRequest: async (requestData: any): Promise<any> => {
    const reqId = `cov_${Date.now()}`;
    const newReq = {
      ...requestData,
      id: reqId,
      status: 'pending'
    ...
    try {
      await setDoc(doc(firestoreDb, 'coveragerequests', reqId), newReq);
    } catch (err) {
      console.warn('[FIRESTORE] createCoverageRequest failed, saving locally:', err);
    }

    // Save locally
    const localCov = localDb.getItem<any[]>('medlink_local_coveragerequests', []);
    localCov.push(newReq);
    localDb.setItem('medlink_local_coveragerequests', localCov);
